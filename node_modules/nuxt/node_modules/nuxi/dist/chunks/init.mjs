...
import { d as defineCommand, c as consola } from '../shared/nuxi.c627b411.mjs';
import { promisify as promisify$3 } from 'node:util';
import 'node:path';
import 'node:process';
import 'node:tty';
import { s as sharedArgs } from '../shared/nuxi.01862219.mjs';
import { r as resolve$1, a as relative, d as dirname$3 } from '../shared/nuxi.dcd5f40f.mjs';
import { existsSync, readdirSync as readdirSync$1, createWriteStream } from 'node:fs';
import { c as commonjsGlobal, a as getDefaultExportFromCjs } from '../shared/nuxi.357b3b4e.mjs';
...
import { b as installDependencies } from '../shared/nuxi.adce9b6f.mjs';
import 'node:http';
import 'node:https';
import 'node:zlib';
import 'node:buffer';
...
const assert$2 = require$$1;
const Buffer$1 = require$$0$3.Buffer;
...
const assert_1 = __importDefault$1(require$$1);
...
const createHttpsProxyAgent$1 = /*@__PURE__*/getDefaultExportFromCjs(dist);
...
async function download(url, filePath, options = {}) {
  ...
  await promisify$3(pipeline)(response.body, stream);
...
async function sendFetch(url, options = {}) {
  if (!options.agent) {
    ...
    if (proxyEnv) {
      options.agent = createHttpsProxyAgent$1(proxyEnv);
    }
  }
  if (options?.headers) {
...
const init = defineCommand({
  ...
  async run(ctx) {
    ...
    const selectedPackageManager = await consola.prompt("Which package manager would you like to use?", {
      type: "select",
...
\u2728 Nuxt project has been created with the \`${template.name}\` template. Next steps:`
    ...
    const nextSteps = [
      !ctx.args.shell && relativeProjectPath.length > 1 && `\`cd ${relativeProjectPath}\``,
