...
async function executeCommand(command, args, options = {}) {
  ...
  const { resolve } = await import('../chunks/index.mjs');
  const execaArgs = command === "npm" || command === "bun" ? [command, args] : ["corepack", [command, ...args]];
...
async function resolveOperationOptions(options = {}) {
  ...
  const packageManager = options.packageManager || await detectPackageManager(options.cwd || process.cwd());
  if (!packageManager) {
...
function getWorkspaceArgs(options) {
  if (!options.workspace) {
    ...
    if (options.packageManager.name === "yarn" && options.packageManager.majorVersion === "1") {
      return ["-W"];
    }
    return [];
  }
      ...
      if (!options.ignorePackageJSON) {
        ...
        if (existsSync(packageJSONPath)) {
          ...
          if (packageJSON?.packageManager) {
            ...
            const packageManager = _packageManagers.find(
...
export { normalizeString as a, installDependencies as b, addDependency as c, detectPackageManager as d, addDevDependency as e, removeDependency as f, ensureDependencyInstalled as g, isAbsolute as i, join as j, normalize as n, resolve as r };
